def rm_scheduler(self):
    processes = sorted(self.processes, key=lambda x: x['period'])  # Sort by period (priority)
    current_time = 0
    gantt_chart = []
    deadline_misses = 0
    total_jobs = 0
    ...
    while current_time < max_time:
        ...
        if selected is not None:
            ...
            if remaining_time[selected] == 0:
                deadline = next_release[selected] - processes[selected]['period']
